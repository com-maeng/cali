...
public class SearchController {
    ...
    @GetMapping("/images")
    public ResponseEntity<?> getHanjaImages(
            @Valid @RequestParam(value = "q") @NotBlank String query,
            @Valid @RequestParam(value = "s") @NotBlank String style,
            @Valid @RequestParam(defaultValue = "0") @Min(value = 0) int offset) {
        return ResponseEntity.ok().body(searchService.getHanjaImages(query, style, offset));
    }
